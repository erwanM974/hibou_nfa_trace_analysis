...
def generate_noise_at_end_mutant(int_name,orig_htf_name,mut_id,max_insert_noise):
    #
    hsf_file = os.path.join(FOLDER_MODEL, "{}.hsf".format(int_name))
    ...
    #
    parent_folder = GlobalTraceKind.ERROR.get_tracegen_folder_name(int_name)
    #
    name = "{}_error_{}".format(orig_htf_name,mut_id)
    command = ["./hibou_label.exe", "mutate_insert_noise", hsf_file, htf_file, "-p", parent_folder, "-n", name, "-m", str(max_insert_noise), "-e"]
    #
    output = subprocess.check_output(command, text=True)
    #print(output)
    #
